...
void BookManager::saveBooks() {
	...
	for (size_t i = 0;i < Book.size();i++) {
		output << i << ";" << Book[i].TitleBook << ";" << Book[i].BookDescr << endl;
	}
	output.close();
}
...
//
unsigned int BookManager::LevenshteinDistance(string s, string t) {
...
void BookManager::findBook() {
	...
	for (size_t i = 0;i < sum;i++) {
		Sort[i].cost = (float)LevenshteinDistance(s, Sort[i].content)/Sort[i].content.length();
	}
